(function () {
  ...
  window.addEventListener(
    ...
    function () {
      ...
      meilleurScore =
        localStorage.getItem(nomLocalStorage) == null
          ? 0
          : localStorage.getItem(nomLocalStorage);
  ...
  function afficherProchaineQuestion() {
    ...
    if (noQuestionEnCours <= QuestionsQuiz.nbQuestions) {
      ...
      divImage.style.backgroundImage =
        "url('" + dossierImg + questionActuelle.images[0] + "')";
      ...
      var textAnim = new AnimTexte(
        QuestionsQuiz.titreQuestion,
        divQuestionTxt,
        60
      );
    } else {
  ...
  function choisirLettre(evt) {
    ...
    lesLettres[indexLettreChoisie].innerHTML =
      lettresChoisies[indexLettreChoisie];
    ...
    if (lettresChoisies.length == questionActuelle.bonneReponse.length) {
      ...
      if (lettresChoisies == questionActuelle.bonneReponse) {
        ...
        afficherFenetre(
          "Excellent!",
          "url('" + dossierImg + questionActuelle.images[1] + "')"
        );
      } else {
  ...
  function afficherFenetre(message, urlImage) {
    ...
    var uneFenetre = new FenetreReponse(
      divImage,
      "fenetre",
      texte,
      urlImage,
  ...
  function finJeu() {
    ...
    //On compose le texte à afficher
    var leTexte =
      "Ton score actuel est de : " +
      score +
      " personnages(s) identifié(s). Ton meilleur résultat est de : " +
      meilleurScore;
